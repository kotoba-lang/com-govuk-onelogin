(ns govuk-onelogin.main
  "Kotodama WASM entrypoint for the Govuk_onelogin clean-room actor (L4) — Clojure port.
  ...
  facts (`govuk_onelogin.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "govuk_onelogin")
(def tier "L4")
...
(def entity-specs
  [{:entity "Filing"      :plural "filings"      :id-prefix "govukone_fil"
    :fields [:filerId :scheme :period :status]  :required [:scheme :period]
    :coerce {}                       :refs {}}
   {:entity "Citizen"     :plural "citizens"     :id-prefix "govukone_cit"
    :fields [:nationalId :jurisdiction]         :required [:jurisdiction]
    :coerce {}                       :refs {}}
   {:entity "Service"     :plural "services"     :id-prefix "govukone_ser"
    :fields [:name :department]                 :required [:name :department]
    :coerce {}                       :refs {}}
   {:entity "Application" :plural "applications" :id-prefix "govukone_app"
    :fields [:citizenId :serviceId :status]     :required [:status]
    :coerce {}                       :refs {:citizenId "Citizen" :serviceId "Service"}}
   {:entity "Dataset"     :plural "datasets"     :id-prefix "govukone_dat"
    :fields [:name :publisher :updatedAt]       :required [:name :publisher]
    :coerce {}                       :refs {}}
   {:entity "Identity"    :plural "identities"   :id-prefix "govukone_ide"
    :fields [:citizenId :assuranceLevel :verified] :required [:assuranceLevel :verified]
    :coerce {:verified :bool}        :refs {:citizenId "Citizen"}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"govuk_onelogin.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "govuk_onelogin-compat" :tier tier :entities entities} 200])
...
(defn start! [] :govuk_onelogin-compat/ready)
