(ns govuk-onelogin.main-test
  "Contract + behavioral test for the govuk-onelogin L4 actor (cljc port).
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "govuk_onelogin-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'govuk-onelogin.main-test)]
